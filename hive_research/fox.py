...
class Fox:
    ...
    def _append_message(self, conversation_id: str, role: str, content: str, **extra: Any) -> None:
        ...
        with self._conv_io_lock:
            ...
            msg = {"role": role, "content": content, "ts": utcnow().isoformat()}
            msg.update(extra)
    ...
    def write_survey_report(
        ...
        md: list[str] = [
            f"# {title}",
            "",
            f"*Generated by Fox on {utcnow().date().isoformat()} — "
        ...
        report_path = reports_dir / f"{slug}_{utcnow():%Y%m%d_%H%M%S}.md"
